...
- Works two ways: clone-and-run, or one-line `curl | bash` (installer fetches `opencode.json` from upstream when not running from a clone).
...
- No telemetry. Installer makes one outbound call to NIM (key validation) and optionally one to fetch `opencode.json` when run via `curl|bash`.
