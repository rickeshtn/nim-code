...
| unsloth/Qwen3-Coder-30B-A3B-Instruct (IQ4_XS), Jackrong/Qwopus3.6-27B-Coder-MTP (Q4_K_S), tvall43/Qwen3.6-14B-A3B-FableVibes (Q8_0), unsloth/gemma-4-26B-A4B-it (UD-IQ4_XS), bartowski/North-Mini-Code-1.0 (IQ4_XS), bytkim/Qwen3.6-27B-MTP-pi-tune (Q4_K_S) | pending | n/a | Pulled-but-not-benched. `ollama pull` failed at the `writing manifest` stage with `permission denied` on `/media/.../ollama-models/manifests/hf.co/<author>/` — the model dir tree got chown'd to `rickeshtn:rickeshtn` while the daemon still runs as user `ollama`. Fix: `sudo chown -R ollama:ollama /media/rickeshtn/<UUID>/ollama-models`, then re-run `bash tools/ollama_pull_targets.sh` extended with these tags. |
