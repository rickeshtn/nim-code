...
4. **`BENCH_LOCAL_TOOLS_MODE` env flag** (added during Qwythos bring-up) — `embed` (default; required for Ollama) splices schemas into the system prompt and drops the `tools` field; `native` keeps the OpenAI `tools` field (required for `llama-server --jinja` against Qwen3.5/Qwythos so the chat template can render tools into the format the model was trained on).
...
| yuxinlu1/gemma-4-12B-agentic-fable5-composer2.5-v2-3.5x-tau2 (Q6_K, local) | **3 / 6** | 578 s | Same 3 / 6 as Q4_K_M — PASS 01 (3 turns / 37 s), 02 (3 / 13), 03 (4 / 173, slow), FAIL 04 (5 / 225), 05 (8 / 87), 99 (5 / 43). **Quant level isn't the lever for this model**; the wall is task complexity (multi-file, large state). |
| yuxinlu1/gemma-4-12B-agentic-fable5-composer2.5-v2-3.5x-tau2 (Q8_0, local) | NO-SCORE × 6 | n/a | Q8 manifest re-pull returned the existing 444 MB broken blob (same sha256, Ollama dedup short-circuited the actual re-download). Local file is corrupt, not a model defect. Fix: `ollama rm <tag>` then remove the blob explicitly before re-pulling, or pull the GGUF directly from HF and import. |
| bartowski/gemma-4-12B-it (Q8_0, local) | NO-SCORE × 6 | n/a | First-party Google base via bartowski's GGUF. Load fails: `unknown model architecture: 'gemma4'`. The official upload uses the newer `gemma4` (no-dash) arch tag; Ollama 0.20.2's bundled llama.cpp only knows the older `gemma-4` (with-dash) variant that the community fable5 fine-tunes use. Run via a recent `llama-server` build directly (same workaround as Qwythos), or wait for an Ollama upgrade. |
| empero-ai/Qwable-9B-Claude-Fable-5 + lordx64/Qwable-v1 (FP16 safetensors → GGUF conversion) | blocked | n/a | Both repos ship safetensors only; our llama.cpp's `convert_hf_to_gguf.py` (Apr 2026 build) errors with `BPE pre-tokenizer was not recognized` on `Qwen3_5ForConditionalGeneration` / `Qwen3_5MoeForConditionalGeneration` — same `qwen35` family that blocked Qwythos under Ollama. Needs a newer convert script that knows the Qwen 3.5 BPE pre-tokenizer hash. |
| [unsloth/Qwen3-Coder-30B-A3B-Instruct](https://huggingface.co/unsloth/Qwen3-Coder-30B-A3B-Instruct-GGUF) (IQ4_XS, local) | **3 / 6** | 126 s | **First local model to clear `05_minigrep`** — the multi-file CLI task every prior local lineup failed. PASS: 02_toposort (4 turns / 19 s), 03_rate_limiter (7 / 21 s), 05_minigrep (4 / 28 s). FAIL: 01_lru (1 turn / 0 calls — model emitted no tool_call on first turn), 04_btree (4 / 44 s), 99_refactor (timeout-ish). Native OpenAI `tools` field works out of the box (Ollama Modelfile declares tool support). MoE with 3 B active per token = fast wall time — total 126 s vs v2's 755 s — even with the FAILed tasks counted. Different failure mix from v2 agentic: clears 05 but misses 01. Ties for top local score with a cleaner profile on hard tasks. |
| Jackrong/Qwopus3.6-27B-Coder-MTP (Q4_K_S, local) | NO-SCORE × 6 | n/a | Loads via Ollama but every chat-completion call returns 500 instantly. Likely the MTP variant tripping a llama.cpp tensor-shape check (same family as the Qwythos `-MTP-` failure earlier today). Needs a recent llama-server (post-MTP-support build) directly. |
| tvall43/Qwen3.6-14B-A3B-FableVibes (Q8_0, local) | NO-SCORE × 6 | n/a | Same: load + first-call fails immediately. Qwen 3.6 arch not yet supported by Ollama 0.20.2's bundled llama.cpp. |
| unsloth/gemma-4-26B-A4B-it (UD-IQ4_XS, local) | NO-SCORE × 6 | n/a | `unknown model architecture: 'gemma4'` — same no-dash arch tag that blocked bartowski's `gemma-4-12B-it Q8` upload. Run via a recent `llama-server` build directly. |
| bartowski/North-Mini-Code-1.0 (IQ4_XS, local) | NO-SCORE × 6 | n/a | `unknown model architecture: 'cohere2moe'` — Cohere's proprietary MoE architecture, not in Ollama's bundled llama.cpp. Would need a llama.cpp build that knows the Cohere Command-A / North family. |
| bytkim/Qwen3.6-27B-MTP-pi-tune (Q4_K_S, local) | NO-SCORE × 6 | n/a | `qwen3next: layer 64 missing attn_qkv/attn_gate projections` — the GGUF declares `qwen3next` arch which Ollama's loader knows partially, but the tensor layout doesn't match (likely an MTP-variant tensor layout the loader doesn't reconstruct). Same workaround as Qwopus / Qwythos: try a recent llama-server. |

### Context-recursion (added 2026-06-25)

`headless_agent.py` grew an opt-in compaction strategy for when the rolling agent transcript exceeds a configurable token budget — **`BENCH_CONTEXT_BUDGET=N`** (default `0` = disabled). When `approx_tokens(messages) > N`, the harness invokes the same model on a subset of older turns to produce a short summary, replaces the middle slice with that summary, and continues the agent loop. If the middle slice itself is more than half the budget, the compaction recurses on it before summarising — that's the "recursion over the context" lever, equivalent to chunked map-reduce on the rolling transcript.

System prompt and the last 2 messages are always preserved. Companion env vars:

- **`BENCH_MAX_TOKENS=N`** (default `8192`) — output generation budget per call. Raised from the previous `4096` because community fine-tunes routinely stuff entire file bodies into a single `<tool_call>write_file(content="...")</tool_call>` and were getting truncated mid-arg.

Demo run: v2 agentic Q4_K_M with `BENCH_CONTEXT_BUDGET=4000`. Compaction fired mid-`05_minigrep` (turn 7, 4146 → recursed 4 levels deep, top-level slice 1006 → 85 tokens ≈ 12× reduction). Score did **not** move: still 3 / 6, same as the baseline. **Confirms the wall on these tasks is model capability, not context budget** — recursion is the right lever when the model has too little KV cache, not when it just can't solve the problem.
| [empero-ai/Qwythos-9B-Claude-Mythos-5-1M](https://huggingface.co/empero-ai/Qwythos-9B-Claude-Mythos-5-1M-GGUF) (Q4_K_M, llama-server, native tools) | **2 / 6** | 194 s | Unblocked by running directly on `llama-server` from `~/Projects/gemma4_wClaude/llama-cpp-turboquant` (Apr 2026 build) at `:18080` with `--jinja`. **Important footgun:** `ollama pull hf.co/empero-ai/Qwythos-9B-Claude-Mythos-5-1M-GGUF:Q4_K_M` resolved to the `-MTP-` variant (5.89 GB blob, metadata `general.name = ... MTP`) which our April-2026 llama.cpp doesn't load (`missing tensor 'blk.32.ssm_conv1d.weight'`). Re-downloading the **non-MTP** `Qwythos-9B-Claude-Mythos-5-1M-Q4_K_M.gguf` (5.63 GB) directly from HF loaded cleanly. PASS: 02_toposort (3 turns / 14 s) and 03_rate_limiter (3 / 14). FAIL: 01_lru burned 10 turns / 9 tool_calls iterating without convergence; 04_btree (4 / 76); 05_minigrep regressed to 0 tool_calls in 1 turn (model emitted plain code); 99_refactor bailed at 3 turns. **Embedded-tools mode (first run, 1/6)** vs **native `tools` field mode (this row, 2/6)** — the OpenAI `tools` field via `--jinja` correctly renders into Qwen3.5's trained function-calling format, so the right local-host mode for Qwythos is `BENCH_LOCAL_TOOLS_MODE=native`. Ollama path remains blocked (`unknown model architecture: 'qwen35'`); re-test when ollama upgrades its bundled llama.cpp. |
